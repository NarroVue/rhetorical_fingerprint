...
POWER_WORDS = {
    ...
    "execute","sovereign","overcome","triumph","crush","defeat",
    "superior","might",
}
...
def highlight_text(text: str, contributions: Dict[str, List[Tuple[str, str]]]) -> str:
    ...
    color_map = {
        ...
        "Us vs Them": "#F7DC6F",  # yellow
        "Legitimacy": "#C39BD3",  # purple
    }
    
    # Create a lookup for word contributions (last-match wins if a word is in multiple categories)
    word_cats = {}
    ...
    for word in words:
        ...
        if word_lower in word_cats:
            ...
            highlighted_words.append(
                ...
                f'border-radius:3px;" title="{category}">{word}</span>'
            )
        else:
...
def create_radar_chart(scores1: Dict[str, float], scores2: Dict[str, float], label1: str, label2: str):
    """Create a line chart comparing two rhetorical fingerprints."""
    ...
    chart = alt.Chart(df).mark_line(point=True).encode(
        ...
        y=alt.Y("Score", scale=alt.Scale(domain=[0, max(df["Score"].max() * 1.2, 1)])),
        color="Text",
        strokeDash="Text"
    ).properties(
        width=400,
...
with st.sidebar:
    ...
    - **Us vs Them**: Words about in-group/out-group identity  
      *(e.g., we, enemy, fight)*
    
    - **Legitimacy**: Words about legal/democratic authority  
      *(e.g., constitution, vote, mandate)*
...
if analyze:
    if not text_a and not text_b:
        ...
        # Line chart comparison if both texts exist
        ...
        **Legend:** 🔴 Power | 🟦 Threat | 🔷 Moral | 🟢 Urgency | 🟡 Us vs Them | 🟣 Legitimacy
        """)
        ...
        if text_a and text_b:
            ...
            st.write(
                f"- Biggest difference is in **{biggest_diff_cat}**: "
                f"{label_b} is {abs(differences[biggest_diff_cat]):.2f} points {direction} per 1000 words"
            )
